...
/// </summary>
public class TrackerPath : CornerstoneObject, ITrackerPath, IDisposable
{
	#region Fields
	...
	/// </summary>
	public TrackerPath() : this(DateTimeProvider.RealTime)
	{
	}
	...
	/// Instantiates a new instance of the class.
	/// </summary>
	public TrackerPath(IDateTimeProvider dateTimeProvider)
	{
	...
	{
		IsCompleted = true;
		CompletedOn = _dateTimeProvider.UtcNow;
	...
	public void TrackAction(Func<string> key, Action<TrackerPath> action)
	{
		...
		result.Name = key();
		result.StartedOn = _dateTimeProvider.UtcNow;
		action(result);
		Children.Add(result);
	}
	...
	private static TrackerPath CreatePathForException(IDateTimeProvider dateTimeProvider, Guid parentId, Exception ex, params TrackerPathValue[] values)
	{
		...
		var response = new TrackerPath(dateTimeProvider)
		{
			ParentId = parentId,
