	...
	where T : ISyncableDatabase, ITrackerDatabase
{
	...
	private void Update(T database, TrackerPathEntity entity, TrackerPath item)
	{
		...
		var configuration = entity.Configuration
			?? (Cache.TryGet(item.Name, out var result)
				? result.Value
		...
		if (configuration == null)
		{
			...
			if (configuration != null)
			{
				Cache.AddOrUpdate(item.Name, configuration);
			}
		}
...
public abstract class TrackerProvider
{
	...
	protected readonly MemoryCache<string, TrackerPathConfigurationEntity> Cache;
	...
	{
		Cache = new MemoryCache<string, TrackerPathConfigurationEntity>();
