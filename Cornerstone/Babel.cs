...
public class Babel
{
	...
	public const string SerializationOnlyConstructor = "This is for serialization only, do NOT use.";
	...
	private readonly ConcurrentDictionary<string, ConcurrentDictionary<string, string>> _dictionaries;
	...
	private Babel()
	{
		...
		_dictionaries.GetOrAdd(Language, _currentDictionary);
	...
	public void Reset()
	{
		...
		// Security Key
		AddOrUpdate("English", BabelKeys.SecurityKeyNotFound, "Card not found.");
		...
		AddOrUpdate("English", BabelKeys.SyncManagerDisabled, "The sync manager is disabled.");
		...
		// Serialization
		AddOrUpdate("English", BabelKeys.SerializationFailedUnsupportedType, "The serialization failed due to unsupported type.");
		AddOrUpdate("English", BabelKeys.DeserializationFailedCrc, "The deserialization failed because CRC is invalid.");
		AddOrUpdate("English", BabelKeys.DeserializationFailedUnsupportedType, "The deserialization failed because a type is not supported.");

		// Source Reflection
		AddOrUpdate("English", BabelKeys.SourceReflectionTypeNotDefined, "The type ({0}) has not been defined for source reflection.");

		// Testing
		AddOrUpdate("English", BabelKeys.TestingShouldBeNull, "The condition was incorrectly not null and should have been null.");
	...
	public static void ThrowIndexOrLengthOutOfRange(int index, int length)
	{
		if (length < 0)
		{
			throw new ArgumentOutOfRangeException(nameof(length), length,
				Tower[BabelKeys.LengthCannotBeNegative]);
		}

		throw new ArgumentOutOfRangeException(nameof(index), index,
			length <= 1
				? Tower[BabelKeys.IndexOutOfRange]
				: Tower[BabelKeys.IndexAndLengthOutOfRange]
		);
	}

	public static void ThrowIndexOutOfRange(int index)
	{
		throw new ArgumentOutOfRangeException(nameof(index), index, Tower[BabelKeys.IndexOutOfRange]);
	}
