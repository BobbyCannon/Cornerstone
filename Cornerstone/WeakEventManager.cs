...
public static class WeakEventManager
{
	...
	private static readonly PresentationList<IWeakEventListener> _listeners;
	...
	static WeakEventManager()
	{
		...
		Listeners = new ReadOnlyPresentationList<IWeakEventListener>(_listeners);
	}
	...
	public static ReadOnlyPresentationList<IWeakEventListener> Listeners { get; }
	...
	/// </summary>
	public static void Add<T, T2>(T source, string eventName, T2 destination, string methodName)
		where T : class
		where T2 : class
	{
		const BindingFlags allFlags = BindingFlags.Instance | BindingFlags.FlattenHierarchy | BindingFlags.NonPublic | BindingFlags.Public;
		var sourceMethodInfo = destination.GetCachedMethod(methodName, [typeof(object), typeof(EventArgs)], allFlags);
		_listeners.Add(new WeakEventListener<T, T2>(source, typeof(T), eventName, destination, sourceMethodInfo.MethodInfo));
	}
	...
	/// <summary>
	/// Registers the given delegate as a handler for the IPresentationList.ListUpdated event
	/// </summary>
	public static void AddPresentationListUpdated<T, TArgs, T2>(T source, T2 destination, EventHandler<PresentationListUpdatedEventArg<TArgs>> handler)
		where T : class, IPresentationList<TArgs>
		where T2 : class
	{
	...
	/// <summary>
	/// Unregisters any previously registered weak event handlers on the given destination object
	/// </summary>
	public static void RemoveByDestination<T>(T destination)
		where T : class
	{
		...
		foreach (var listener in _listeners)
		{
			...
			else if (listener.Destination == destination)
			{
	...
	/// </summary>
	public static void RemoveBySource<T>(T source)
		where T : class
	{
		var toRemove = new List<IWeakEventListener>();
		foreach (var listener in _listeners)
