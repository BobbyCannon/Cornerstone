...
public class Screen
{
	...
	{
		var info = new NativeScreen.MONITORINFOEX();
		NativeScreen.GetMonitorInfo(new HandleRef(null, monitor), info);
	...
	/// </summary>
	public static bool MultipleScreenSupport => (uint) NativeScreen.GetSystemMetrics(80) > 0U;
	...
	/// <summary>
	/// Gets the current display refresh rate in hertz for this screen. Zero when unknown.
	/// </summary>
	public int RefreshRate
	{
		get
		{
			var mode = new NativeScreen.DEVMODE
			{
				dmSize = (ushort) Marshal.SizeOf<NativeScreen.DEVMODE>()
			};

			if (NativeScreen.EnumDisplaySettings(
					DeviceName,
					(int) NativeScreen.DEVMODE_SETTINGS.ENUM_CURRENT_SETTINGS,
					ref mode))
			{
	...
	public static Rectangle VirtualScreenSize =>
		MultipleScreenSupport
			...
			? new Rectangle(
				NativeScreen.GetSystemMetrics(76),
				NativeScreen.GetSystemMetrics(77),
				NativeScreen.GetSystemMetrics(78),
				NativeScreen.GetSystemMetrics(79)
			)
	...
	{
		var monitorEnumCallback = new NativeScreen.MonitorEnumCallback();
		var monitorEnumProc = new NativeScreen.MonitorEnumProc(monitorEnumCallback.Callback);
		NativeScreen.EnumDisplayMonitors(NativeScreen.NullHandleRef, null, monitorEnumProc, IntPtr.Zero);
