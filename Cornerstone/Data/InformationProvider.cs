	...
	where T : IBindable, IUpdateable, new()
{
	...
	protected InformationProvider(IDispatcher dispatcher) : base(dispatcher)
	{
		...
		// Command
		StartMonitoringCommand = new RelayCommand(_ => StartMonitoringAsync(), _ => !IsMonitoring);
		StopMonitoringCommand = new RelayCommand(_ => StopMonitoringAsync(), _ => IsMonitoring);
	}
