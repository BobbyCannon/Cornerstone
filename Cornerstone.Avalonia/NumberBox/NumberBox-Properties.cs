...
public partial class NumberBox
{
	...
	static NumberBox()
	{
		AcceptsExpressionProperty = AvaloniaProperty.Register<NumberBox, bool>(nameof(AcceptsExpression));
		DescriptionProperty = AvaloniaProperty.Register<NumberBox, string>(nameof(Description));
		FocusableProperty.OverrideDefaultValue<NumberBox>(true);
		HeaderProperty = HeaderedContentControl.HeaderProperty.AddOwner<NumberBox>();
		HeaderTemplateProperty = HeaderedContentControl.HeaderTemplateProperty.AddOwner<NumberBox>();
		InnerLeftContentProperty = TextBox.InnerLeftContentProperty.AddOwner<NumberBox>();
		IsWrapEnabledProperty = AvaloniaProperty.Register<NumberBox, bool>(nameof(IsWrapEnabled));
		LargeChangeProperty = RangeBase.LargeChangeProperty.AddOwner<NumberBox>();
		MaximumProperty = RangeBase.MaximumProperty.AddOwner<NumberBox>(
		...
		MinimumProperty = RangeBase.MinimumProperty.AddOwner<NumberBox>(
			new StyledPropertyMetadata<double>(
				double.MinValue,
				coerce: (ao, d1) =>
				{
		...
		PlaceholderTextProperty = TextBox.PlaceholderTextProperty.AddOwner<NumberBox>();
		SelectionBrushProperty = TextBox.SelectionBrushProperty.AddOwner<NumberBox>();
		SimpleNumberFormatProperty = AvaloniaProperty.Register<NumberBox, string>(nameof(SimpleNumberFormat));
		SmallChangeProperty = RangeBase.SmallChangeProperty.AddOwner<NumberBox>();
		SpinButtonPlacementModeProperty = AvaloniaProperty.Register<NumberBox, NumberBoxSpinButtonPlacementMode>(nameof(SpinButtonPlacementMode));
		TextAlignmentProperty = TextBlock.TextAlignmentProperty.AddOwner<NumberBox>();
		TextProperty = AvaloniaProperty.RegisterDirect<NumberBox, string>(nameof(Text),
			x => x.Text, (x, v) => x.Text = v, defaultBindingMode: BindingMode.TwoWay);
		ValidationModeProperty = AvaloniaProperty.Register<NumberBox, NumberBoxValidationMode>(nameof(ValidationMode));
		ValueProperty = RangeBase.ValueProperty.AddOwner<NumberBox>(
			new StyledPropertyMetadata<double>(
	...
	/// </summary>
	/// <remarks>
	/// .NET does not have all the formatting stuff from WinUI/WinRT, thus doing fancy things
