...
public partial class NumberBox : TemplatedControl
{
	...
	public void StepValue(double change)
	{
		...
		if (!double.IsNaN(newVal))
		{
			...
			// to change the number box value.
			MoveCaretToTextEnd();
		}
	}
	...
	protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
	{
		...
		if (_textBox != null)
		{
			...
			_textBox.AddHandler(KeyUpEvent, OnNumberBoxKeyUp, RoutingStrategies.Tunnel);
		}
	...
	protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
	{
		...
		else if ((change.Property == LargeChangeProperty)
	...
	{
		// Due to the popup being light dismissable, we need to ensure we can reopen the
		// popup if focus never leaves the control, but we click back on it
		// Do this in preview because TextBox will handle pointer event.
		if ((SpinButtonPlacementMode == NumberBoxSpinButtonPlacementMode.Compact)
	...
	private void UpdateHeaderPresenterState()
	{
		...
		PseudoClasses.Set(s_pcHeader, showHeader);
	}
	...
	private void UpdateTextToValue()
	{
		...
		finally
		{
			...
			// Only move caret if we're focused, otherwise it triggers a bring into view event
