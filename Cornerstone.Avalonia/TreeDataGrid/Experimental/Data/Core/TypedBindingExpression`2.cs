...
public class TypedBindingExpression<TIn, TOut>
	: LightweightObservableBase<BindingValue<TOut>>,
	...
	where TIn : class
{
	...
	private IDisposable _rootSourceSubscription;
	...
	protected override void Deinitialize()
	{
		...
		_rootSourceSubscription?.Dispose();
		_rootSourceSubscription = null;
	...
	protected override void Initialize()
	{
		...
		_rootSourceSubscription = _rootSource.Subscribe(RootChanged);
		_flags |= Flags.Initialized;
	}
	...
	private void ChainPropertyChanged(object sender)
	{
		...
		{
			StopListeningToChain(changedMemberIndex);
			ListenToChain(changedMemberIndex);
		}
	...
	{
		if ((_root?.TryGetTarget(out var t) == true)
			&& t is { } target
			&& _read is { } read)
		{
			try
		...
		if ((_flags & Flags.RootHasFired) != 0)
		{
	...
	private struct Link
	{
		...
		public readonly Func<TIn, object> Eval;
