...
internal readonly struct IndexRange : IEquatable<IndexRange>
{
	...
	private static readonly IndexRange _invalid;
	...
	static IndexRange()
	{
		_invalid = new(int.MinValue, int.MinValue);
	}
		...
		IList<IndexRange> added = null)
	{
		...
		for (var i = 0; (i < ranges.Count) && (range != _invalid); ++i)
		{
			...
			if (range.Intersects(existing) || range.Adjacent(existing))
			{
				...
				range = range.End <= existing.End ? _invalid : new IndexRange(existing.End + 1, range.End);
			}
