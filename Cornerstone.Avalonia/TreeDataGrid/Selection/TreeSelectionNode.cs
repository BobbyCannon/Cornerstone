...
internal class TreeSelectionNode<T> : SelectionNodeBase<T>
{
	...
	public int CommitDeselect(IndexRange range)
	{
		return CommitDeselect(range.Begin, range.End);
	}

	public int CommitSelect(IndexRange range)
	{
		return CommitSelect(range.Begin, range.End);
	}

	public TreeSelectionNode<T> GetChild(int index)
	{
		return index < _children?.Count ? _children[index] : null;
	}

	public TreeSelectionNode<T> GetOrCreateChild(int index)
	{
		if (GetChild(index) is { } result)
		{
			return result;
		}

		var childCount = ItemsView?.Count ?? Math.Max(_children?.Count ?? 0, index);
