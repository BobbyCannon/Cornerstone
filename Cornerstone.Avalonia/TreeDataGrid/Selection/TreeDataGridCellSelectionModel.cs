...
public class TreeDataGridCellSelectionModel<TModel>
	: TreeDataGridCellSelectionModel,
		ITreeDataGridCellSelectionModel<TModel>, ITreeDataGridSelectionInteraction where TModel : class
{
	#region Fields

	private bool _columnsChanged;
	private Point _pressedPoint;
	private (int x, int y) _rangeAnchor;
	...
	{
		get => _selectedRows.Source;
		set => _selectedRows.Source = value;
	}
	...
	public bool IsSelected(CellIndex index)
	{
		return IsSelected(index.ColumnIndex, index.RowIndex);
	}
		...
		bool rightButton = false)
	{
		...
		if (rightButton)
		{
			if (!IsSelected(columnIndex, modelIndex) && !treeDataGrid.QueryCancelSelection())
			{
				Select(columnIndex, rowIndex, modelIndex);
			}
		}
...
public class TreeDataGridCellSelectionModel
	: ITreeDataGridSelectionInteraction
{
	#region Fields

	protected static readonly Point _invalidPoint;
	...
	static TreeDataGridCellSelectionModel()
	{
