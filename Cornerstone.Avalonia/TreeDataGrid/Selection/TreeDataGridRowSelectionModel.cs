	...
	where TModel : class
{
	...
	static TreeDataGridRowSelectionModel()
	{
	...
	public IReadOnlyList<TModel> SelectedItemsInOrder
	{
		get
		{
			...
			{
				if ((rowIndex >= 0)
					&& (rowIndex < _source.Rows.Count)
					&& _source.Rows[rowIndex].Model is TModel m)
				{
					list.Add(m);
				}
			}
	...
	public override void Deselect(IndexPath index)
	{
		var rowIndex = _source.Rows.ModelIndexToRowIndex(index);
		if (rowIndex >= 0)
		{
			_selectionOrder.Remove(rowIndex);
		}
	...
	protected override void Select(IndexPath index, bool updateRangeAnchorIndex)
	{
		var rowIndex = _source.Rows.ModelIndexToRowIndex(index);
		if (rowIndex >= 0)
		{
			// Add to end if not already there (preserves order)
			if (!_selectionOrder.Contains(rowIndex))
			{
				_selectionOrder.Add(rowIndex);
			}
		}

		base.Select(index, updateRangeAnchorIndex);
	}
		...
		TreeDataGridRow focused)
	{
		...
		if (direction
			is NavigationDirection.First
			or NavigationDirection.Last)
		{
	...
	void ITreeDataGridSelectionInteraction.OnKeyDown(TreeDataGrid sender, KeyEventArgs e)
	{
		...
		// todo: change home/end to require ctrl
	...
	void ITreeDataGridSelectionInteraction.OnPointerPressed(TreeDataGrid sender, PointerPressedEventArgs e)
	{
		...
		if (!e.Handled && pointerSupportSelectionOnPress)
		{
			var hitVisual = sender.GetVisualAt(point, v => v is TreeDataGridRow { RowIndex: >= 0 });
			var row = hitVisual as TreeDataGridRow;

			if ((row == null) && e.Source is Control source)
			{
				row = source.FindAncestorOfType<TreeDataGridRow>(true);
				if (row is { RowIndex: < 0 })
				{
					row = null;
				}
			}
		...
		// Double-click/tap handling – use the same combined approach for consistency
		if (e.ClickCount >= 2)
		{
			TreeDataGridRow doubleClickRow = null;

			var hitVisual = sender.GetVisualAt(point, v => v is TreeDataGridRow { RowIndex: >= 0 });
			doubleClickRow = hitVisual as TreeDataGridRow;

			if ((doubleClickRow == null)
				&& e.Source is Control source)
			{
				doubleClickRow = source.FindAncestorOfType<TreeDataGridRow>(true);
				if (doubleClickRow is { RowIndex: < 0 })
				{
					doubleClickRow = null;
				}
			}

			var treeRow = doubleClickRow?.Rows[doubleClickRow.RowIndex];
			if (treeRow is IExpander expander)
			{
	...
	void ITreeDataGridSelectionInteraction.OnPreviewKeyDown(TreeDataGrid sender, KeyEventArgs e)
	{
		...
		if (e.Key is Key.PageDown or Key.PageUp && (sender.RowsPresenter?.Items != null))
		{
			...
			if (childrenCount > 0)
			{
				...
				var selectedIndex = sender.Rows!.ModelIndexToRowIndex(SelectedIndex);
				if (e.Key == Key.PageDown)
				{
					...
					if (isIndexSet &&
						...
						sender.TryGetRow(selectedIndex) is { } row &&
						IsRowFullyVisibleToUser(row))
					{
	...
	private void OnSelectionChanged(object s, TreeSelectionModelSelectionChangedEventArgs<TModel> e)
	{
	...
	{
		var found = false;
		for (var i = candidatePattern.Length == 1 ? selectedRowIndex + 1 : selectedRowIndex; i <= (_source.Rows.Count - 1); i++)
		{
		...
		bool rightButton = false)
	{
		...
		else if ((SelectedIndex != modelIndex) || (Count > 1))
		{
			if (!treeDataGrid.QueryCancelSelection())
			{
				// This is a new primary selection → reset order to just this row
				_selectionOrder.Clear();
				SelectedIndex = modelIndex; // this will trigger Select() → adds to order
			}
		}
	}
