...
public abstract class TreeDataGridPresenterBase<TItem> : Border
{
	...
	public Control BringIntoView(int index, Rect? rect = null)
	{
		...
		if (this.IsAttachedToVisualTree())
		{
			...
			var elementRect = Orientation == Orientation.Horizontal ? new Rect(anchorU, 0, scrollToElement.DesiredSize.Width, scrollToElement.DesiredSize.Height) : new Rect(0, anchorU, scrollToElement.DesiredSize.Width, scrollToElement.DesiredSize.Height);
		...
		try
		{
			...
			for (var i = 0; i < _realizedElements.Count; ++i)
			{
				...
				if (e is not null)
				{
					...
					var rect = orientation == Orientation.Horizontal ? new Rect(u, 0, sizeU, finalSize.Height) : new Rect(0, u, finalSize.Width, sizeU);
	...
	protected virtual (int index, double position) GetElementAt(double position)
	{
		return (-1, -1);
	}
	...
	protected virtual double GetElementPosition(int index)
	{
		return -1;
	}
	...
	/// pass. This pass is only run if InitialMeasurePassComplete returns
	/// true.
	/// </remarks>
				...
				for (var i = 0; i < count; ++i)
				{
					...
					if (HasInfinity(previous))
					{
						...
						viewport.measuredV = Math.Max(
							viewport.measuredV,
							Orientation == Orientation.Horizontal ? e.DesiredSize.Height : e.DesiredSize.Width);
					}
				}
			}
	...
	protected virtual bool NeedsFinalMeasurePass(
		int firstIndex,
		IReadOnlyList<Control> elements)
	{
		return false;
	}
	...
	protected virtual void OnEffectiveViewportChanged(object sender, EffectiveViewportChangedEventArgs e)
	{
		...
		// viewport.
		Viewport = e.EffectiveViewport.Size == default ? s_invalidViewport : Intersect(e.EffectiveViewport, new(Bounds.Size));
	...
	private Rect EstimateViewport(Size availableSize)
	{
		...
		while (c is not null)
		{
			if (!c.Bounds.Equals(default) && c.TransformToVisual(this) is { } transform)
			{
	...
	private static bool HasInfinity(Size s)
	{
