...
public abstract class EntityFrameworkDatabase : DbContext, IDatabase
{
		...
		: base(startup)
	{
		...
		if (settings != null)
		{
			DatabaseSettings.UpdateWith(settings);
		}
	}

	static EntityFrameworkDatabase()
	{
		WarmUpSqliteForKnownIssueEF001();
	}
	...
	public abstract string LastDatabaseMigrationId { get; }
	...
	{
		return Database.ProviderName?.Contains("Sqlite") == true ? DatabaseType.Sqlite : DatabaseType.SqlServer;
	}
	...
	public virtual bool IsDatabaseMigrated()
	{
		...
		// 1. Fast Path: Check if the specific last migration is already applied via direct SQL
		if (!string.IsNullOrWhiteSpace(LastDatabaseMigrationId))
		{
			try
			{
				var query = GetDatabaseType() == DatabaseType.Sqlite
					? "SELECT EXISTS (SELECT 1 FROM MigrationHistory WHERE MigrationId = @p0) AS Value"
					: "SELECT CASE WHEN EXISTS (SELECT 1 FROM [system].[MigrationHistory] WHERE [MigrationId] = @p0) THEN 1 ELSE 0 END AS Value";

				var isLastApplied = Database.SqlQuery<int>(FormattableStringFactory.Create(query, LastDatabaseMigrationId)).FirstOrDefault() == 1;
			...
			catch (Exception)
			{
				...
				// If the SQL check fails, fall through to the heavy check to ensure correctness
			}
		}
	...
	private static void WarmUpSqliteForKnownIssueEF001()
	{
		try
		{
			// Forces the static constructor of SqliteConnection to run.
			// The ApplicationData.Current probe happens here.
			//_ = typeof(Microsoft.Data.Sqlite.SqliteConnection).TypeInitializer;

			// Alternative that also works:
			using var _ = new SqliteConnection();
		}
		catch (TypeInitializationException ex)
			when (ex.InnerException is InvalidOperationException ioe
				&& (uint) ioe.HResult is 0x80073D54 or 0x8000000D)
		{
			// Expected on unpackaged Windows apps – swallow it.
			// The type is now initialized; future SqliteConnection / EF calls are fine.
		}
		catch (InvalidOperationException ioe)
			when ((uint) ioe.HResult is 0x80073D54 or 0x8000000D)
		{
			// Same HResult – also expected.
		}
	}
