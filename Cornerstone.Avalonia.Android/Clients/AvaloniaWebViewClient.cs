...
internal class CornerstoneWebViewClient : WebViewClient
{
	...
	{
		_webViewAdapter.OnNavigationCompleted(new WebViewNavigationEventArgs { Request = new Uri(url) });
	...
	{
		_webViewAdapter.OnNavigationStarted(new WebViewNavigationEventArgs { Request = new Uri(url) });
