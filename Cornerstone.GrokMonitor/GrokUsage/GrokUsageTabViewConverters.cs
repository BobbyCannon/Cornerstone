...
public static class GrokUsageConverters
{
	...
	private static Color ResolveThemeColor(int themeIndex)
	{
		...
		var key = "ThemeColor" + index.ToString("00", CultureInfo.InvariantCulture);
		var color = ResourceService.GetColor(key);
		return color;
	}
	...
	private static IBrush ToTokenHeatBrush(IEnumerable<object> values)
	{
		...
		var themeColor = ResolveThemeColor(heat.ThemeIndex);
		if (themeColor == default)
		{
			return Brushes.Transparent;
		}

		return new ImmutableSolidColorBrush(Color.FromArgb(heat.A, themeColor.R, themeColor.G, themeColor.B));
	}
