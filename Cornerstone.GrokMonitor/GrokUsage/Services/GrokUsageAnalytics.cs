...
public static class GrokUsageAnalytics
{
	...
	/// <summary>
	/// Sessions at or above this total use full heat (ThemeColor09 clamp).
	/// </summary>
	...
	private const int HeatThemeIndexMax = 9;
	...
	/// <summary>
	/// Periods that have at least one subscription-model inference in range.
	/// Weeks with only session summaries (rotated CLI log) are omitted.
	/// </summary>
	public static IReadOnlyList<UsagePeriodOption> FilterPeriodsWithTokenUsage(
		...
		IReadOnlyList<InferenceUsage> inferences)
	{
		...
		if ((inferences == null) || (inferences.Count == 0))
		{
			return [];
		}

		var result = new List<UsagePeriodOption>();
		foreach (var option in options)
		{
			var hasUsage = false;
			foreach (var inference in inferences)
			{
				if (IsInHalfOpenRange(inference.Timestamp, option.PeriodStart, option.PeriodEnd)
					&& IsSubscriptionGrokModel(inference.ModelId))
				{
					hasUsage = true;
					break;
				}
	...
	{
		return hasAllocatedUsage ? $"{percent:0.00}%" : string.Empty;
	}
	...
	public static string FormatCompactTokens(double tokens)
	{
		...
		{
			return $"{tokens / 1_000_000_000:0.00}B";
		}

		if (value >= 1_000_000)
		{
			return $"{tokens / 1_000_000:0.00}M";
		}

		if (value >= 1_000)
		{
			return $"{tokens / 1_000:0.00}K";
		}
	...
	/// <summary>
	/// Session-row heat from total tokens and thresholds.
	/// None when disabled or under soft; ThemeColor00 at soft; ThemeColor09 at hot and above.
	/// Theme index plus alpha for UI converters and tests (no Avalonia types).
	/// </summary>
		...
		long hotThreshold)
	{
		...
		var themeIndex = (int) Math.Round(t * HeatThemeIndexMax);
