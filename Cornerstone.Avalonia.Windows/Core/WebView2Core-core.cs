...
using Cornerstone.Avalonia.Windows.Helpers;
...
partial class WebView2Core
{
	...
	private void ClearBlazorWebViewCompleted(CoreWebView2 coreWebView2)
	{
	...
	private CoreWebView2ControllerOptions CreateCoreWebView2ControllerOptions(CoreWebView2Environment environment)
	{
	...
	private Task PrepareBlazorWebViewStarting(IVirtualBlazorWebViewProvider provider, CoreWebView2 coreWebView2)
	{
	...
	private void SetEnvirmentDefaultBackground(Color color)
	{
		Environment.SetEnvironmentVariable("WEBVIEW2_DEFAULT_BACKGROUND_COLOR", color.Name);
	}

	private protected string GetHeaderString(IDictionary<string, string> headers)
	{
