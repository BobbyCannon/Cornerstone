...
public class SampleClientSyncManager : SyncManager
{
	#region Fields
	...
	/// <inheritdoc />
	public SampleClientSyncManager(ISyncClientProvider clientSyncClientProvider, IServerSyncClientProvider serverSyncClientProvider,
		...
		return Process("Accounts", options =>
			{
				...
				options.AddFilter(new SyncRepositoryFilter<ClientAccount>());
	...
	public SyncSession SyncAddresses(Func<bool> testUntil = null, TimeSpan? waitFor = null, Action<SyncSession> postAction = null)
	{
		return Process("Addresses", options =>
			{
				options.ResetFilters();
				options.AddFilter(new SyncRepositoryFilter<ClientAddress>());
		...
		return Process("All", options =>
			{
				...
				options.AddFilter(new SyncRepositoryFilter<ClientAccount>());
				options.AddFilter(new SyncRepositoryFilter<ClientAddress>());
				options.AddFilter(new SyncRepositoryFilter<ClientLogEvent>());
