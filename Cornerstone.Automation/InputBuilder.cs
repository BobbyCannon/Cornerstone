...
using Cornerstone.Automation.Internal.Inputs;
...
public class InputBuilder
{
	...
	/// <returns> This <see cref="InputBuilder" /> instance. </returns>
	public InputBuilder AddMouseButtonDrag(MouseButton button, int x, int y)
	{
		// Move relative movement of the mouse cursor
		var input = new InputTypeWithData { Type = (uint) InputType.Mouse };
		input.Data.Mouse.Flags = (uint) ToMouseButtonDownFlag(button);

		_inputList.Add(input);

		// Move relative movement of the mouse cursor
		input = new InputTypeWithData { Type = (uint) InputType.Mouse };
		input.Data.Mouse.Flags = (uint) (ToMouseButtonUpFlag(button) | MouseFlag.Move | MouseFlag.MoveNoCoalesce);
