...
public abstract class ChromiumBrowser : Browser
{
	...
	/// </summary>
	private const string DebugArgument = "--remote-debugging-port={0} --profile-directory=Cornerstone --enable-automation --no-first-run --disable-extensions --disable-features=PrivacySandboxSettings4";
	...
	{
		return UtilityExtensions.WaitUntil(() => _socketResponses.ContainsKey(id.ToString()), Application.Timeout, 1);
	}
