...
public class XmlParser
{
	...
	public string? AttributeValue => State == ParserState.AttributeValue ? _data.Span.Slice(_attributeValueStart).ToString() : null;
	...
	public int? CurrentValueStart =>
		State switch
		{
			ParserState.StartElement => _elementNameStart,
			ParserState.StartAttribute => _attributeNameStart,
			ParserState.AttributeValue => _attributeValueStart,
			_ => null
		};

	public int? ElementNameEnd => State >= ParserState.StartElement ? _elementNameEnd : null;

	public bool IsInClosingTag { get; private set; }

	public int NestingLevel => _containingTagStart.Count;

	public int ParserPos { get; private set; }
	...
	public string? GetParentTagName(int level)
	{
		...
		return m.Success ? m.Value.Substring(1) : null;
	}
	...
	public static XmlParser Parse(ReadOnlyMemory<char> data, int start, int end)
	{
		...
		while ((rv.ParserPos < start) && rv.ParseChar())
		{
		}
	...
	private bool ParseChar()
	{
		...
		{
			State = ParserState.StartElement;
			IsInClosingTag = (_data.Span.Length > (i + 1)) && (span[i + 1] == '/');
