...
/// </summary>
[Guid(CornerstoneConstants.CornerstoneFactoryEditorGuidString)]
internal sealed class EditorFactory : IVsEditorFactory, IDisposable
{
	private readonly CornerstonePackage _package;
	...
	public EditorFactory(CornerstonePackage package)
	{
		_package = package;
	}
		...
		out int pgrfCDW)
	{
		...
		pguidCmdUI = CornerstoneConstants.CornerstoneFactoryEditorGuid;
		pgrfCDW = 0;
		...
		var textLines = GetTextBuffer(punkDocDataExisting);
	...
	private static ProjectItem GetExtensionObject(IVsHierarchy hierarchy, uint itemId)
	{
		ThreadHelper.ThrowIfNotOnUIThread();
