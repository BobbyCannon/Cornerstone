...
public partial class WeaverForPropertyChanged
{
	...
	{
		foreach (var data in node.PropertyData)
		{
	...
	private MethodReference FindTypeEquality(PropertyData propertyData)
	{
		...
		_methodCache[fullName] = equality;
		return equality;
	}

	private static TypeReference GetBaseType(TypeReference typeReference)
	{
	...
	private MethodReference GetStaticEquality(TypeReference typeReference)
	{
		...
		if (UseStaticEqualsFromBase)
		{
			while ((equality == null) &&
					...
					!_methodCache.TryGetValue(typeReference.FullName, out equality))
			{
		...
		typesChecked.ForEach(typeName => _methodCache[typeName] = equality);
