...
public partial class WeaverForPropertyChanged
{
	...
	private static OnChangedTypes GetBeforeAfterOnChangedMethodType(MethodDefinition method, out string argumentTypeFullName)
	{
	...
	private static string GetMethodWarning(TypeNode notifyNode, OnChangedMethod onChangedMethod)
	{
		...
		if (foundProperty.CustomAttributes.ContainsAttribute(Constants.DoNotNotifyAttribute))
		{
			return $"{baseMessage} as {propertyName} is attributed with [DoNotNotify].";
		}

		if (foundProperty.CustomAttributes.ContainsAttribute(Constants.OnChangedMethodAttribute))
		{
	...
	private ICollection<OnChangedMethod> GetOnChangedMethods(TypeNode notifyNode)
	{
		...
		foreach (var propertyDefinition in notifyNode.DeclaredProperties)
		{
			...
			foreach (var attribute in propertyDefinition.CustomAttributes.GetAttributes("Cornerstone.Weaver.OnChangedMethodAttribute"))
			{
			...
			if (InjectOnPropertyNameChanged
				&& !hasCustomMethods
				&& methods.TryGetValue("On" + propertyDefinition.Name + "Changed", out var defaultMethod))
			{
	...
	private static PropertyDefinition GetProperty(MethodReference methodRef, string propertyName)
	{
	...
	private void ProcessOnChangedMethods(List<TypeNode> notifyNodes)
	{
	...
	private void ValidateOnChangedMethod(TypeNode notifyNode, OnChangedMethod onChangedMethod)
	{
		...
		{
			return notifyNode.PropertyData
