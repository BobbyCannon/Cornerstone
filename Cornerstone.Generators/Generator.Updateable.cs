...
public partial class Generator
{
	...
	public static void ProcessUpdateable(CSharpCodeBuilder builder, SourceTypeInfo sourceTypeInfo)
	{
		...
		//if (classInfo.ClassSymbol.Name == "CompareViewModel")
		//{
		//	Debugger.Break();
		//}
		...
		// Get all other IUpdateable<> interfaces
	...
	private static void GenerateUpdateableIncludedProperties(CSharpCodeBuilder builder, IList<UpdateablePropertyOrder> updateableProperties)
	{
		...
		// Group keys by their property lists
		var groupedKeys = keysInOrder.Where(key => dictionary.ContainsKey(key) && (dictionary[key].Count > 0)).GroupBy(key => string.Join(",", dictionary[key].OrderBy(x => x))).Select(g => (Keys: g.ToList(), Members: dictionary[g.First()].OrderBy(x => x).ToList())).ToList();

		foreach (var group in groupedKeys)
		{
			// Write case statements for grouped keys
	...
	private static UpdateableDetails[] GetOtherUpdateables(INamedTypeSymbol typeSymbol)
	{
		...
		foreach (var implementedInterface in typeSymbol.Interfaces)
		{
			...
			// Check if UpdateWith is implemented in typeSymbol or its base types
	...
	{
		var properties = GetAllMembers(typeInfo.TypeSymbol).OfType<IPropertySymbol>().ToList();
		...
		foreach (var property in properties)
		{
			var attributes = property
				.GetAttributes()
				.Where(a => a.AttributeClass?.MetadataName == UpdateableActionAttribute)
				.ToArray();
		...
		return allUpdateableProperties.OrderBy(x => x.Order).ThenBy(x => x.Name).ToList();
	}
	...
	private static bool HasUpdateWithMethod(INamedTypeSymbol typeSymbol, INamedTypeSymbol typeArg)
	{
		...
		// todo: detect if it will be generated
		return false;
	}
	...
	{
		// Check if the interface is IUpdateable
		if (interfaceSymbol.MetadataName is UpdateableName)
		{
			return true;
		}

		// Check if the interface is IUpdateable<T> (generic with one type parameter)
		if (interfaceSymbol.MetadataName is UpdateableGenericName
			&& (interfaceSymbol.TypeArguments.Length == 1))
		{
			// Check if the type argument T matches the typeSymbol itself
	...
	private static void ProcessUpdateWith(CSharpCodeBuilder builder, INamedTypeSymbol classType, string methodModifier, bool invokeBase, IList<UpdateablePropertyOrder> members)
	{
		...
		{
			// Value == true == IsReadOnly
			builder.IndentWriteLine(member.IsReadOnly
	...
	private static void ProcessUpdateWithForObject(CSharpCodeBuilder builder, INamedTypeSymbol classInfo, IEnumerable<INamedTypeSymbol> updateables)
	{
		...
		//builder.IndentWriteLine($"{classInfo.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)} value => UpdateWith(value, settings),");
	...
	private static bool ShouldInvokeBase(INamedTypeSymbol typeSymbol)
	{
		foreach (var current in GetBaseTypes(typeSymbol))
		{
			...
			{
				// Check for attribute to see if we should not invoke base.
	...
	private static bool ShouldOverrideBase(INamedTypeSymbol typeSymbol, INamedTypeSymbol parameterType)
	{
		foreach (var current in GetBaseTypes(typeSymbol))
		{
			// Check for UpdateableAttribute.InvokeBase to ensure allowed
			...
			// todo: support finding generated methods
			// note: could look up interfaces?

			// Check if UpdateWith is declared
