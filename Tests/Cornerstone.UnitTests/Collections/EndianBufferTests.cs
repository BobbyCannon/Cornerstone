...
[TestClass]
public class EndianBufferTests : CornerstoneUnitTest
{
	#region Methods
	...
	public void ShouldWriteCorrectlyForInt16()
	{
		...
		AreEqual(expected, actual, () => actual.ToHexString());
	...
	public void WritesShouldException()
	{
		...
		foreach (var scenario in scenarios)
		{
			ExpectedException<CornerstoneException>(() => scenario.WriteInt16(short.MaxValue), "Failed to write the int16 value.");
			ExpectedException<CornerstoneException>(() => scenario.WriteInt32(int.MaxValue), "Failed to write the int32 value.");
			ExpectedException<CornerstoneException>(() => scenario.WriteInt64(long.MaxValue), "Failed to write the int64 value.");
			ExpectedException<CornerstoneException>(() => scenario.WriteUInt16(ushort.MaxValue), "Failed to write the uint16 value.");
			ExpectedException<CornerstoneException>(() => scenario.WriteUInt32(uint.MaxValue), "Failed to write the uint32 value.");
			ExpectedException<CornerstoneException>(() => scenario.WriteUInt64(ulong.MaxValue), "Failed to write the uint64 value.");
		}
	}
