...
public class CornerstoneObjectTests : CornerstoneUnitTest
{
	#region Methods
	...
	public void HasChangesWithCombinedIncludeAndExcludeSettings()
	{
		...
		IsTrue(account.HasChanges(settings));

		// Now reset and only change Roles (which is both included and excluded)
		account.ResetHasChanges();
		account.Roles = "User";
		IsFalse(account.HasChanges(settings));
	}
	...
	public void HasChangesWithEmptySettings()
	{
		...
		IsTrue(account.HasChanges(IncludeExcludeSettings.Empty));
	}
	...
	public void HasChangesWithExcludeSettings()
	{
		...
		IsFalse(account.HasChanges(excludeName));

		account.EmailAddress = "john@test.com";
		IsTrue(account.HasChanges(excludeName));
	}
	...
	public void HasChangesWithIncludeSettings()
	{
		...
		IsFalse(account.HasChanges(rolesOnly));
	}
	...
	public void InitializeIsIdempotent()
	{
		...
		{
			if (e.PropertyName == nameof(CornerstoneObject.IsLifecycleInitialized))
			{
				changedCount++;
			}
		};
	...
	[TestMethod]
	public void InitializeSetsFlagAndRaisesEvents()
	{
		var obj = new TestObject();
		obj.InitializeLifecycle();
		Assert.IsTrue(obj.IsLifecycleInitialized());
	...
	public void NotificationsCanBeDisabled()
	{
		...
		obj.PropertyChanged += (_, _) => changedCount++;

		obj.DisablePropertyChangeNotifications();
		...
		Assert.IsTrue(obj.IsLifecycleInitialized());
		Assert.IsTrue(obj.IsLifecycleLoaded());
		Assert.IsTrue(obj.IsLifecycleStarted());
	}
	...
	{
		var test = new SetFlagTest();
		var actual = test.GetChangedProperties();
		AreEqual([], actual);
		test.SetChangeFlags();
		actual = test.GetChangedProperties();
		AreEqual([nameof(SetFlagTest.Foo)], actual);
	}
	...
	[TestMethod]
	public void UnloadCorrectlySetsFlagToFalse()
	{
		var obj = new TestObject();
		obj.LoadLifecycle();
		Assert.IsTrue(obj.IsLifecycleLoaded());
		...
		obj.UnloadLifecycle();
		Assert.IsFalse(obj.IsLifecycleLoaded());
	}
	...
	[SourceReflection]
	public class SetFlagTest : CornerstoneObject
	{
		#region Properties
		...
		{
			SetChangedProperties(nameof(Foo));
		}

		#endregion
	}

	private sealed class TestObject : CornerstoneObject
	{
	}
